...
import { ADMIN, CART_ABANDONER, CUSTOMER, login } from './helpers';
...
test.describe('cart, checkout, and order history', () => {
    test('a customer can add to cart, check out, and see the order in their history', async ({
        ...
        await page.locator('label:has-text("Set as default")').locator('input').check();
        ...
        await expect(page.locator('form input[type=radio]:checked')).toHaveCount(1);
